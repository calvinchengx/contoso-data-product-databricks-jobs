...
storing one. Nothing here is committed, and on `DATABRICKS_TARGET=real` the
...
def resolve(secret: str) -> str:
    ...
    if not t.seed_secrets_allowed:
        ...
        raise SystemExit(
            f"DATABRICKS_TARGET=real: {secret!r} must arrive in the environment "
            f"as {env_name(secret)}, resolved from the customer's secret scope. "
